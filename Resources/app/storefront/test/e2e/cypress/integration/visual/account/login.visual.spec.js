...
describe('Account: Visual tests login as customer', () => {
    ...
    it('@visual: check appearance of login with wrong credentials', () => {
        ...
        cy.takeSnapshot('[Account] Login', page.elements.loginCard, {widths: [375, 768, 1920]});

        cy.get('#loginMail').typeAndCheckStorefront('test@example.com');
        cy.get('#loginPassword').typeAndCheckStorefront('shopware');

        cy.takeSnapshot('[Account] Login with right credentials', page.elements.loginCard, {widths: [375, 1920]});
        ...
        cy.takeSnapshot('[Account] Login Success', {widths: [375, 768, 1920]});
    });
});
