...
describe('Account: Address page', () => {
    ...
    it('@visual: update address page', () => {
        ...
        cy.takeSnapshot('[Account] Address page', {widths: [375, 768, 1920]});
        ...
        cy.takeSnapshot('[Address] Fill in Create new address form', {widths: [375, 768, 1920]});

        cy.get('.address-form-submit').click();
        cy.get('.alert-success .alert-content').contains('Address has been saved.');
        cy.takeSnapshot('[Address] Address Created', {widths: [375, 768, 1920]});
        ...
        cy.get('.alert-success .alert-content').contains('Default address has been changed.');
        cy.takeSnapshot('[Address] Address - Set default billing address', {widths: [375, 768, 1920]});
        ...
        cy.get('.address-form-submit').scrollIntoView();
        cy.takeSnapshot('[Address] Fill in edit address form', {widths: [375, 768, 1920]});

        cy.get('.address-form-submit').click();
        cy.get('.alert-success .alert-content').contains('Address has been saved.');
        cy.takeSnapshot('[Address] Change address form', {widths: [375, 768, 1920]});

        cy.get('.address-card').contains('Delete').click();
        cy.get('.alert-success .alert-content').contains('Address has been deleted.');
        cy.takeSnapshot('[Address] Delete address', {widths: [375, 768, 1920]});
    });
});
