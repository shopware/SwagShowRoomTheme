...
describe('Paypal: Checkout', () => {
    beforeEach(() => {
        return cy.setToInitialState()
            ...
            .then(() => {
                // Ensure that the PayPal Iframes are loaded before the tests start
                cy.intercept('GET', '**/smart/buttons*').as('paypalSmartButtons');
                cy.visit('/');
                cy.scrollTo('bottom');
                cy.wait('@paypalSmartButtons');
            })
    });
    ...
    it('@workflow @paypal: should have Paypal checkout button for default settings', () => {
        ...
        cy.intercept('GET', '**/smart/buttons*').as('paypalSmartButtons');
        cy.wait('@paypalSmartButtons').its('response.statusCode').should('equal', 200);
    ...
    it('@workflow @paypal: should have Paypal checkout button in Offcanvas cart', () => {
        ...
        cy.get('.offcanvas-cart div[data-swag-paypal-installment-banner="true"]').should('be.exist');
    });
    ...
    it('@workflow @paypal: should have Paypal checkout in checkout register page', () => {
        ...
        cy.get('div[data-swag-paypal-express-button="true"]').scrollIntoView().should('be.visible');
        cy.get('div[data-swag-paypal-installment-banner="true"]').should('be.exist');
    });
