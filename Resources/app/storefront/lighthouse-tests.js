...
async function getNavUrl(browser) {
    ...
    await page.goto(`${APP_URL}`);
    const menuButton = await page.$('.header-row .d-sm-block .nav-main-toggle-btn');
    menuButton.click();

    await page.waitForSelector('li.navigation-offcanvas-list-item');

    let secondNavLinkUrl = await page.$eval('li.navigation-offcanvas-list-item:nth-of-type(2n)', el => el.href);
...
async function main() {
    ...
    const testCasesLoggedIn = {
        ...
        loggedInCategoryPage: async () => `${APP_URL}`,
